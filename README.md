...
ログインはユーザー名＋PIN（メール認証は使わない）。内部では
`ユーザー名@kakeibo.local` を合成アドレスに、PINをパスワードにしてSupabase Authへ
サインアップ/サインインする（`src/storage.js`）。

セットアップ（1回だけ）:
1. https://supabase.com で無料プロジェクトを作成
2. Authentication → Providers → Email を有効化し、"Confirm email" はオフ
   （合成アドレスには実際のメールが届かないため確認をオフにする必要がある）
3. SQL Editor で以下を実行:
```sql
...
-- RLSのポリシーとは別に、ログインユーザー(authenticated)へテーブルの
-- 操作権限(GRANT)を渡す必要がある。これが無いと "permission denied for
-- table kv" になる。
...
`VITE_SUPABASE_URL`、`VITE_SUPABASE_ANON_KEY`を登録してください。デプロイ時に
公開接続情報がアプリへ組み込まれ、PC・スマホでは同じユーザー名とPINで
ログインできます。anon keyはブラウザで利用する公開キーであり、service_role keyは使用しないでください。

ローカル開発では `.env.example` を `.env.local` にコピーして値を設定できます。
ログイン後は変更時、起動時、オンライン復帰時、画面復帰時、および表示中30秒ごとに自動同期します。

## iPhoneから明細CSVを取り込む
取込画面の「共有から1タップ取込を設定する」に、iOSショートカットの作成手順を用意している。
初回だけ共有シートに表示する「家計簿CSV取込」を作成すると、以後は銀行サイトでCSVを
ダウンロードし、ファイルの共有メニューからショートカットを選ぶだけで、解析済みの取込画面が開く。
ショートカットを使わない場合も、「ダウンロードしたCSVを選ぶ」からFilesの最近使った項目を選択できる。
